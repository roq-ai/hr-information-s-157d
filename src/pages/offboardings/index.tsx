...
import { getOffboardings, deleteOffboardingById } from 'apiSdk/offboardings';
...
function OffboardingListPage() {
  ...
  const handleView = (id: string) => {
    if (hasAccess('offboarding', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/offboardings/view/${id}`);
    }
  };
    ...
    <AppLayout>
      <Box bg="white" p={4} rounded="md" shadow="md">
        <Flex justifyContent="space-between" mb={4}>
          ...
          {hasAccess('offboarding', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/offboardings/create`} passHref legacyBehavior>
                  ...
                  <Tr cursor="pointer" onClick={() => handleView(record.id)} key={record.id}>
                    ...
                    {hasAccess('employee', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>
                        <Link as={NextLink} href={`/employees/view/${record.employee?.id}`}>
                          {record.employee?.personal_details}
                        </Link>
                      </Td>
                    )}
                    ...
                    <Td>
                      {hasAccess('offboarding', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                        <NextLink href={`/offboardings/edit/${record.id}`} passHref legacyBehavior>
                          <Button
